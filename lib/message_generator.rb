  ...
  def balance_message(food_stamp_balance, optional_balances = {})
    if language == :spanish
      if optional_balances[:cash]
        ...
        balance_message = "Hola! El saldo de su cuenta de estampillas para comida es www.ebt.ai/#{food_stamp_balance}."
      end
    else
      if optional_balances[:cash]
        balance_message = "Hi! Your food stamp balance is #{food_stamp_balance} and your cash balance is #{optional_balances[:cash]}."
      else
        balance_message = "Hi! Your EBT balance is #{food_stamp_balance}. The calander is updated and available here. www.ebt.ai/##{food_stamp_balance.gsub(/[!$."]/,'')}."
      end
    end
    balance_message
  end
